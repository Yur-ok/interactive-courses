...
#### PHP
* https://returntrue.win/ (en) - Задача передать в фукнцию корректные параметры, чтобы получить **```true```**. 
Чуть ниже поля ввода указывается идеальный вариант ответа (число соответствует количеству символов). 
Вы, конечно, можете решить задачу указав более длинный вариант ответа. Но тогда вы так и не узнаете фишки языка!
<details>
